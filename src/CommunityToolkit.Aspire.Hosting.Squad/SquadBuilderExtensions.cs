...
public static class SquadBuilderExtensions
{
    ...
    /// the Aspire dashboard, and injects a Hybrid PATH+URI connection string that downstream
    /// services can consume. The Squad resource is a logical dashboard/resource-graph entry;
    ...
    /// <summary>
    /// Opt-in process-spawning lifecycle. PLANNED for v1.1+.
    ///
    /// In v1.0 the Squad CLI must be started externally (in a separate
    /// terminal). This method is reserved so future opt-in process
    /// management can be added without breaking the v1.0 API surface.
    /// </summary>
    /// <param name="builder">The <see cref="IResourceBuilder{SquadResource}"/>.</param>
    /// <param name="cliPath">Optional path to the squad CLI executable.</param>
    /// <param name="cliArgs">Optional additional CLI arguments.</param>
    /// <param name="waitForReady">Whether to wait for the CLI to signal readiness before completing the lifecycle hook.</param>
    /// <returns>The <see cref="IResourceBuilder{SquadResource}"/> for further configuration.</returns>
